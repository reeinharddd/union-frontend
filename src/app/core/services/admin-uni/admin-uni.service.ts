...
export class AdminUniService extends BaseService {
      ...
      catchError((error: any) => {
        ...
        if (error.status === 500) {
          ...
          if (
            ...
            throw new Error(
              'Error de base de datos: Conflicto de ID en tabla usuarios. Contacta al administrador técnico.',
            );
          } else if (errorMessage.includes('Error al crear alumno')) {
            console.error('❌ Error general al crear alumno');
            throw new Error(
              'Error al registrar el alumno. Verifica los datos e inténtalo nuevamente.',
            );
          }
          ...
          if (
            ...
            throw new Error(
              '❌ No tienes permisos para registrar alumnos. Verifica tu rol de Administración Universitaria',
            );
          }
        }
  ...
  getStudentsByUniversity(universityId: number): Observable<Student[]> {
    ...
    return this.handleRequest(
      this.apiClient.get<Student[]>(
        `${API_ENDPOINTS.USERS.BASE}?rol_id=2&universidad_id=${universityId}`,
      ),
