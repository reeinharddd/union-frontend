...
export class EventListComponent implements OnInit {
  ...
  private loadEvents(): void {
    ...
    this.apiClient.get<Event[]>('/eventos').pipe(
      tap((events: Event[]) => {
  ...
  private loadEventTypes(): void {
    ...
    this.apiClient.get<EventType[]>('/event-types').pipe(
      tap((types: EventType[]) => {
  ...
  private loadUniversities(): void {
    ...
    this.apiClient.get<University[]>('/universidades').pipe(
  ...
  applyFilter(filterValue: string): void {
    ...
    switch (filterValue) {
      ...
      case 'my_university':
        filtered = allEvents.filter(event => 
          event.universidad_id === userUniversityId
        );
        break;
      case 'virtual':
        filtered = allEvents.filter(event => 
          event.modalidad === 'virtual' || event.modalidad === 'hibrida'
        );
        break;
      case 'presencial':
        filtered = allEvents.filter(event => 
          event.modalidad === 'presencial' || event.modalidad === 'hibrida'
        );
        break;
      case 'registered':
  ...
  registerForEvent(eventId: number): void {
    ...
    this.asistenciaService.registrarseEvento(eventId).pipe(
      tap((asistencia) => {
  ...
  isEventRegistered(eventId: number): boolean {
    ...
    if (localRegistered !== serviceRegistered) {
      console.warn('⚠️ Discrepancia en estado de registro:', { eventId, localRegistered, serviceRegistered });
  ...
  unregisterFromEvent(eventId: number): void {
    this.asistenciaService.cancelarRegistroPorEvento(eventId).pipe(
      tap(() => {
